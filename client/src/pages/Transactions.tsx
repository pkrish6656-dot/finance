...
import { useTransactions, useCreateTransaction, useUploadTransactions } from "@/hooks/use-finance";
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage and track your recent spending.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button 
            variant="outline" 
            className="rounded-xl border-border/50 shadow-sm hover-elevate"
            onClick={() => fileInputRef.current?.click()}
            disabled={uploadTx.isPending}
          >
            {uploadTx.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Upload className="w-4 h-4 mr-2" />}
            Upload CSV
          </Button>

          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            <DialogTrigger asChild>
              <Button className="rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground shadow-md hover-elevate">
            ...
            <DialogContent className="sm:max-w-[425px] rounded-2xl">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. Starbucks" {...field} className="rounded-xl" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g. Coffee" {...field} className="rounded-xl" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <Button type="submit" className="w-full rounded-xl mt-4" disabled={createTx.isPending}>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center text-muted-foreground flex flex-col items-center">
              ...
              <p className="text-sm mt-1">Upload a CSV or add one manually to get started.</p>
            </div>
          ) : (
            <Table>
